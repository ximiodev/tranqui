...
+ (NSString *)uid {
    return [[[IAPDevice alloc] initWithKey:@"deviceUID"] uid];
}
 ...
 */
- (void)save {
    if (![IAPDevice valueForUserDefaultsKey:_uidKey]) {
        [IAPDevice setValue:self.uid forUserDefaultsKey:_uidKey];
    }
    if (![IAPDevice valueForKeychainKey:_uidKey service:_uidKey]) {
        [IAPDevice setValue:self.uid forKeychainKey:_uidKey inService:_uidKey];
    }
}
